...
var global={
    ...
    getcartinfo:function(pid){
        ...
        var serverurl= base_url +"cart/getcart";
        $.ajax(  
                {  
                    type: "POST", 
                    url: serverurl,
...
//                                             console.log(f['data']);
                                        if(f['data']['numbers']!=='undefined'  && f['data']['total']!=='undefined'){
                                            $('#cart_number').text(f['data']['numbers']);
...
//                                                c='共<span class="cod00000">'+f['data']['numbers']+'</span>件商品<br>共计<span class="cod00000 w500">¥'+f['data']['total']+'</span><a href="'+$('#base_url').val()+'cart/"><span class="v30_btred v30hmcf_gc">去购物袋结算</span></a>';
    ...
    /* 添加到购物车*/
    addtocart:function(pid,num,sid,come_source){
        var base_url = $('#base_url').val();
        if(base_url == undefined){
              return false;
        }
        var serverurl= base_url +"cart/ajxaddcart";
        $.ajax(  
                {  
                    type: "POST", 
                    url: serverurl,
                    data:{'ajxtype':'1','id':pid,'num':num,'store_off_id':sid,'source':come_source},
                    asynic:false,
                    beforeSend:function(){
                    },
                    success: function(originalRequest)
...
//                                             $('.goods_add_popup').show();
    ...
    getlogininfo:function(){
        ...
        //生成唯一cookie，做独立访客标示 2016.08.24
        genuniqueuid:function(){
           Cookie.SetCookie('uniqueuid',Math.floor(Math.random()*1000000));
        },
        //数据统计
        statistics:function() {
            var base_url = $('#base_url').val();
            ...
            var serverurl= base_url +"home/statistics";
            $.post(serverurl, {}, function(res){});
        }
};
